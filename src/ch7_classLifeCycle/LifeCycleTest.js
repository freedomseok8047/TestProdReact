...
class LifeCycleTest extends Component {
    ...
    // 여기서, 부모로 부터 전달 받은 props 내용이 담겨져 있음
    // 초기화를 하면서, 자식 입장에서 사용이 된다.
    // props.color 가 들어 있다.
    constructor(props) {
        // 클래스가 혼자 동작 못하고, 반드시 어떤 부모의 상속을 받아야함
        // 부모를 초기화 해줘야함 
        super(props)
        console.log("1번째 constructor 호출")
    }

    // 생명주기 2번째,getDerivedStateFromProps
    // 마운트 할때도, 업데이트 할때도 호출 -> 변경시 비교함 
    ...
    // 4번째 호출 : 마운트가 완료됨
    componentDidMount(){
        console.log("4번째 호출 : componentDidMount 마운트가 완료됨")
    }
  ...
  getSnapshotBeforeUpdate(prevProps, prevState) {
    console.log(
        "스냅샷 확인 : getSnapshotBeforeUpdate 호출. ",
        prevProps.color,
        this.props.color
      );
